...
contract AccidentHandler20220715V3 is Initializable, Adminable {
    ...
    mapping(address => mapping(address => uint256)) public userRetrievableTokenMap;
    // [user]: [token]: amount # retrieved
    mapping(address => mapping(address => uint256)) public userRetrievedTokenMap;
    // [token]: amount # retrievable
    mapping(address => uint256) public remainTokenMap;

    uint256 public endingAt;

    event Retrieve(address user, address token, uint256 amount);
    ...
    function initialize(address admin_, uint256 endingAt_) public initializer {
        require(admin_ != address(0), "Can't set admin to zero address");
        _setAdmin(admin_);
        setEndingAt(endingAt_);
    }

    function retrievables(address[] calldata tokens) external view returns (uint256[] memory) {
    ...
    function retrieved(address[] calldata tokens) external view returns (uint256[] memory) {
        uint256[] memory amounts = new uint256[](tokens.length);
        for (uint256 i; i < tokens.length; i++) {
    ...
    function setRecords(Record[] calldata records_) external onlyAdmin {
        for (uint256 i; i < records_.length; i++) {
            ...
            require(
                userRetrievableTokenMap[record.user][record.token] == 0 &&
    ...
    function setEndingAt(uint256 endingAt_) public onlyAdmin {
    ...
    function retrieveTokens(address[] calldata tokens) external {
        ...
        for (uint256 i; i < tokens.length; i++) {
            ...
            uint256 amount = userRetrievableTokenMap[msg.sender][tokens[i]];
            if (amount > 0) {
