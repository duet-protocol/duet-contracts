...
contract StrategyKeeper is KeeperCompatibleInterface, Ownable{
    ...
    function removeStrategyList(uint index) external onlyOwner {
        uint len = strategyList.length;
        if (index >= len) return;
    ...
    function _toBytes20(bytes memory _b, uint _offset) internal pure returns (bytes20) {
        bytes20 out;
