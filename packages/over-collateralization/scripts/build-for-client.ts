...
void (async () => {
  ...
  for (const name of await readdir(deploymentsPath)) {
    ...
    for (const contract of contracts) {
      ...
      if (!exporting.has(`${contract.class}__factory`))
        throw new Error(`[${name}] Can not resolve "${contract.class}" factory`);
    ...
    await new Promise<void>((res, rej) => {
      spawn(resolve(root, '../..', 'node_modules/.bin/tsc'), { cwd: packageRoot, stdio: 'inherit' }).on(
        'close',
        (code) => {
    ...
    await Promise.all(
      (
        await readdir(`${packageRoot}/src/typechain`)
      ).map((name) => {
    ...
    await new Promise<void>((res, rej) => {
      spawn('tar', ['-czf', `../${packageName}.tar.gz`, '.'], { cwd: packageRoot, stdio: 'inherit' }).on(
        'close',
        (code) => {
          if (code) rej(new Error(`Failed by "${code}"`));
          else res();
