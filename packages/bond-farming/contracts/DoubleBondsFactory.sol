...
contract DoubleBondsFactory is Ownable, AccessControl{
    ...
    function new_double_bonds (address _borrowtoken,address _staketoken, uint256 _start, uint256 _duration, uint256 _phasenum,uint256 _principal,uint256 _interestone,address _debtor,address _clearer,uint256 _stakenum) external onlyRole(ISSUE_ROLE){
        IERC20 token = IERC20(_staketoken);    
        require(token.balanceOf(msg.sender)>=_stakenum,"factory:no balance");
        DoubleBond doublebondCon = new DoubleBond(_borrowtoken, _staketoken, _start, _duration, _phasenum, _principal, _interestone, _debtor, _clearer);
        DoubleBondPool memory dbp = DoubleBondPool(address(doublebondCon),_borrowtoken,_staketoken,_duration, _debtor,_clearer);
        doublebonds.push(dbp);
        token.transferFrom(msg.sender, address(doublebondCon), _stakenum);
    }

    function renewal_double_bond(address doubleBond, uint256 _phasenum,uint256 _principal,uint256 _interestone,uint256 _stakenum) external onlyRole(DEFAULT_ADMIN_ROLE){
