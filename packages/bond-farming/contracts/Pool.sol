...
contract Pool {
    ...
    mapping(address => mapping(address => uint256)) public rewardAvailable;

    struct EpochInfo {
        uint256 accPerShare; //Accumulated rewards per share, times SCALE
        uint256 epochPerSecond; // for total deposit
    }
    ...
    function getPassed() internal view returns (uint256) {
        uint256 endTs;
    ...
    function updateUser(
        ...
        for (uint256 i = 0; i < epoches.length; i++) {
            ...
            if (liq) {
                ...
                rewardAvailable[user][epoches[i]] +=
                    ((deposits[user] * epinfo.accPerShare) / SCALE) -
                    rewardDebt[user][epoches[i]];
            }
    ...
    function deposit(address user, uint256 amount) external onlyFarming {
        updatePool();
        uint256 newDeposit = deposits[user] + amount;
