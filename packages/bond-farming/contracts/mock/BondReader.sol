...
contract BondReader {
  ...
  function duetValue(uint amount) public view returns(uint256 value) {
    (address oracle, , ) = controller.getValueConf(duet);
    uint price = IUSDOracle(oracle).getPrice(duet);
  ...
  function poolPendingAward(address pool, address user) external view returns(
    address[] memory epochs, uint256[] memory awards, uint[] memory ends, string[] memory symbols) {
  ...
  function myBonds(address user) external view returns(uint256[] memory balances,
    uint[] memory ends,
    uint[] memory prices,
    uint[] memory totals, string[] memory symbols
    ) {
