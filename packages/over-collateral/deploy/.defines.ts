...
export async function isProxiedContractDeployable(
  ...
  const proxyAdmin = await getProxyAdminAddress((await get(name)).address)
  // not proxy or not deployed yet
  if (proxyAdmin === '0x0000000000000000000000000000000000000000') {
    return true;
  }
  return deployer.toLowerCase() === proxyAdmin.toLocaleLowerCase();
}

export async function getProxyAdminAddress(contractAddress: string) {
  // bytes32(uint256(keccak256('eip1967.proxy.admin')) - 1) see: https://eips.ethereum.org/EIPS/eip-1967
  const proxyAdminSlotKey = '0xb53127684a568b3173ae13b9f8a6016e243e63b6e8ee1178d6a717850b5d6103';
  const proxyAdminSlotValue = await ethers.provider.getStorageAt(contractAddress, proxyAdminSlotKey);
  return ethers.utils.getAddress(`0x${proxyAdminSlotValue.substring(26)}`)
}

export async function getProxyImplementAddress(contractAddress: string) {
  ...
  return ethers.utils.getAddress(`0x${proxyImplementSlotValue.substring(26)}`)
}
