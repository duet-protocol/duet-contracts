...
import { intersection, isNumber, last, range } from 'lodash'
...
export function parseOjWithBigNumber(record: Record<string, unknown> | any) {
  const newRecord: any = {}
  for (const key of Object.keys(record)) {
    const val = record[key]
    if (isBigNumber(val)) {
      newRecord[key] = (val as { toString: () => string }).toString()
      continue
    }
    newRecord[key] = val
  }
